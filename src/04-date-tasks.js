 ...
 *    Date.UTC(2016,3,5,21, 0) => Math.PI/2 resultData
 */
function angleBetweenClockHands(date) {
  const resultData = new Date(date);
  // eslint-disable-next-line max-len
  let injection = Math.abs(0.5 * (60 * resultData.getUTCHours() - 11 * resultData.getUTCMinutes())) % 360;
  injection = injection > 180 ? 360 - injection : injection;
  return (injection * Math.PI) / 180;
}
